...
- `apps/web/src/styles/tokens.css` — the `@theme` block. This is where literals live.
- `apps/web/src/styles/globals.css` — reset only.
- SVG icon path data (`d=`, `viewBox`) — geometry, not design tokens.

Everything under `apps/web/src/components/` and `apps/web/src/app/` must be clean.
...
```
apps/web/src/components/listing/RatingCard.tsx:22: #222222 → colors.ink → use text-ink
apps/web/src/components/listing/HeroGrid.tsx:8: rounded-[14px] → rounded.md → use rounded-md
apps/web/src/components/ui/Badge.tsx:11: #f0f0f0 → NO TOKEN — measure from reference, add to tokens.css, flag for DESIGN.md
```
