...
```
src/components/gallery/Lightbox.tsx:42: HIGH: focus not restored on ESC — trigger element ref never captured. Store the activeElement before open, focus it in the cleanup.
src/components/listing/Nav.tsx:19: MED: icon-only save button has no accessible name. Add aria-label.
```
